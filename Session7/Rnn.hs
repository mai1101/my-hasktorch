...
rnnHiddenDim :: Int
rnnHiddenDim = 64  -- RNN memorize size
...
data RNNSpec = RNNSpec { -- Elman.hsからの引用
    in_features :: Int, 
    hidden_features :: Int
} deriving (Show, Eq, Generic)
...
-- h_t = tanh(W_ih * x_t + W_hh * h_t-1 + b)  (参考：Elman.hs)
...
rnnCalc rnnLayer input = 
    let seqLen = size 1 input --length of sentence
        batch_size = size 0 input --batch size
...
forWard model inputIdxes = 
    ...
    in output --mse
    --in Torch.logSoftmax (Dim 1) output --nll
    ...
    let -- 1つのレビューを処理する関数
        ...
        extractAndClean rev = 
            let cleanedtext = B.pack $ map toLowerW8 $ filter (not . isUnncessaryChar) (encode (text rev))
                ...
                wordList = Data.List.take 30 $ B.split (head $ encode " ") cleanedtext    --delete long setence                        
        ...
        -- padding
        maxLen = maximum (map (length . fst) pairs) --文の単語数の最大を知る
...
main = do
    ...
    let initEmbSpec = EmbeddingSpec {wordNum = wordNumValue, wordDim = wordDimension}    
        ...
        initMlpSpec = MLPSpec {feature_counts = [rnnHiddenDim, 10, 1], nonlinearitySpec = Torch.tanh}
        modelSpec = ModelSpec { embeddingSpec = initEmbSpec, rnnSpec = initRnnSpec, mlpSpec = initMlpSpec }
    initModel <- initialize modelSpec embeddingPath --1.c use pre-trained params
